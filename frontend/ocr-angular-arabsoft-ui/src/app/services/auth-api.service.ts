...
export class AuthApiService {
  ...
  history(limit = 50): Observable<HistoryResponse> {
    return this.http.get<HistoryResponse>(`${this.baseUrl}/history?limit=${limit}`, {
      headers: this.authHeaders(),
    });
  }
  ...
  getApiKey(): string {
    return localStorage.getItem('ocr_api_key') || 'dev-key-123';
  }
  ...
  isSimpleUser(): boolean {
    return this.getRole() === 'simple_user';
  }
  ...
  hasRole(roles: Array<UserRole | 'superuser'>): boolean {
    ...
    if (roles.includes('superuser') && user.is_superuser) {
      return true;
    }
  ...
  canExtract(): boolean {
    return this.hasRole(['admin', 'operator', 'simple_user']);
  }
  ...
  canViewProfile(): boolean {
    return this.hasRole(['admin', 'operator', 'simple_user', 'viewer']);
  }
