...
class PipelinePreprocessor:
    ...
    def prepare_for_cin(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        Simplifié : seul un redressement (deskew) est appliqué. Ni
        débruitage ni CLAHE — l'image reste la plus proche possible de
        l'original pour l'extraction ROI, qui s'est révélée plus fiable
        sur image non retouchée que sur image contrastée artificiellement.

        CIN ROI extraction est sensible au resize, deskew et à la
        localisation de la carte : on garde ces étapes, mais on retire
        tout renforcement de contraste (CLAHE).
        """
        ...
        quality = compute_quality(deskewed)
        ...
        return {
            ...
            # Image utilisée par le pipeline CIN : resize + deskew seulement.
            "image": deskewed,
            "base_image": deskewed,
            "variants": {
                "base": deskewed,
