...
Fix log v2.2:
  • _extract_family_name_from_text: the "before_label" pattern (value
    BEFORE اللقب) is now only tried as a fallback when the "normal" pattern
    (اللقب THEN value) found nothing. On non-deduplicated multi-engine
    concatenated raw_text, "before_label" could accidentally match an
    artefact sequence created by juxtaposed OCR passes, and its higher
    base score (36) made it win over a correct "normal" match (30) that
    had already found the true family name.
  • _looks_like_family_name: reject candidates containing a duplicated
    token (e.g. "يعامج زينب يعامج") — a strong signal of concatenation
    artefact rather than a real Tunisian family name.
"""
...
from app.services.cin_rules import TN_PLACES, fix_reversed_name_token

log = get_logger(__name__)
settings = get_settings()

# Individual words making up any known Tunisian place name (e.g. "حمام"
# and "الأنف" from "حمام الأنف"). Used to reject name candidates that are
# actually a fragment of the birth-place value leaking into a name field
# via a spatial box mismatch or OCR-shuffled text (observed in production:
# first_name resolved to "حمام", a fragment of birth place "حمام الأنف",
# because a box near a corrupted "الام" label anchor was picked up instead
# of the real first-name box). A single word being a place-name component
# is a strong signal it does not belong in a person-name field.
_PLACE_NAME_WORDS = {word for place in TN_PLACES for word in place.split()}
...
class CINPipelineRunner:
    ...
    def _clean_family_candidate(self, raw: Any) -> Optional[str]:
        ...
        for tok in text.split():
            ...
            if not clean or clean in stop_tokens:
                break
            # EasyOCR/Paddle sometimes emit a name token character-mirrored
            # (e.g. "رهام" for "ماهر") — inconsistently, so only flip when
            # the reversed form matches a known common name and the OCR'd
            # form doesn't (see fix_reversed_name_token).
            tokens.append(fix_reversed_name_token(clean))
        ...
        # Reject candidates that are actually a birth-place fragment leaked
        # into the family-name field (e.g. spatial box mismatch picking a
        # box near "حمام الأنف" instead of the real family-name box).
        if any(t in _PLACE_NAME_WORDS for t in tokens):
            return None
    ...
    def _extract_family_name_from_text(self, text: str) -> Optional[str]:
        ...
        FIX v2.2: the "before_label" (value BEFORE اللقب) and
        "weak_before_label" patterns are now tried ONLY as a fallback, when
        the strong "normal" pattern (اللقب THEN value) found nothing at all.

        Why: raw_text is a concatenation of several OCR engine passes,
        never deduplicated (see _best_available_raw_text). On real cards
        this sometimes produces artefact sequences where a token from one
        OCR pass happens to sit right before "اللقب" from another pass,
        purely by concatenation order — not because it is really the value
        preceding the label on the card. Because "before_label" had a
        higher base score (36) than "normal" (30), such an artefact could
        outscore and override a perfectly correct "normal" match, producing
        duplicated/garbled family names (e.g. "يعامج زينب يعامج" instead of
        "جماعي"). Restricting "before_label" to a fallback-only role fixes
        this while preserving its usefulness for genuinely RTL-reordered
        OCR output where "normal" truly finds nothing.
        """
        candidates: List[Tuple[int, str, str]] = []

        # Strong normal pattern: label then value, cut before الاسم/date/place.
        # This is the primary, most reliable pattern and is always tried first.
        for m in re.finditer(
            ...
            if val:
                score = 30 + len(val.split())
                # raw_text concatenates multiple non-deduplicated OCR passes
                # (see _best_available_raw_text), so two "normal" matches can
                # tie on score even though only one is a genuine label-value
                # pair; the other can sit in a jumbled/looped fragment where
                # the same value repeats nearby (FIX v2.2's own example,
                # "...يعامج زينب يعامج اللقب زينب...", wrongly tying against
                # a correct earlier "اللقب جماعي" and winning the tie-break
                # on "later occurrence"). If the captured value already
                # appears again in the text immediately preceding this
                # match, that repetition is a strong signal of such an
                # artefact — penalize it so a clean, non-repeated match wins.
                preceding = text[max(0, m.start() - 30):m.start()]
                if val in preceding:
                    score -= 20
                candidates.append((score, val, f"normal:{m.group(0)}"))

        # FIX v2.2: fallback patterns only — never allowed to outrank a
        # correct "normal" match, so only attempted when candidates is empty.
        if not candidates:
            # Strong value-before-label pattern: family immediately before
            # اللقب, first name immediately before الاسم.
            for m in re.finditer(
            ...
            # Weak fallback: value before اللقب only if it is not preceded by
            # obvious OCR markers or reversed fragments.
            for m in re.finditer(
    ...
    def _clean_first_name_candidate(self, raw: Any) -> Optional[str]:
        ...
        # Reject candidates that are actually a birth-place fragment leaked
        # into the first-name field (e.g. spatial box mismatch picking a
        # box near "حمام الأنف" instead of the real first-name box —
        # observed in production returning first_name="حمام").
        if any(t in _PLACE_NAME_WORDS for t in tokens):
            return None
    ...
    def _extract_birth_place_from_raw_context(self, raw_text: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        For each مكانها-labeled value found, both its OCR word order and the
        word-reversed form are tried (same EasyOCR/Paddle RTL box-ordering
        issue that _extract_names_from_cin_text already guards against for
        names): a multi-word place like "القلعة الكبرى" whose OCR boxes were
        ordered left-to-right on a right-to-left line would otherwise come
        back as "الكبرى القلعة" instead of the true reading order.
        """
        text = self._normalize_cin_arabic_text(raw_text)
        text = re.sub(r"[^\u0600-\u06FF0-9\s]", " ", text)
        text = re.sub(r"\s+", " ", text).strip()

        candidates = self._birth_place_candidates_in_text(text)
        if not candidates:
            return None, None
    ...
    def _birth_place_candidates_in_text(self, text: str) -> List[Tuple[int, int, str, str]]:
        ...
        # Merged with the full TN_PLACES gazetteer (app.services.cin_rules):
        # this list used to be a small, separately-maintained 12-entry subset
        # that missed many real Tunisian governorates/towns (e.g. "المهدية"),
        # so places outside that subset never got the known-place bonus/
        # multi-word recognition and could come out truncated or wrong.
        # Sorted longest-first so a compound place ("حزوة توزر") is matched
        # before a shorter place name that happens to be its substring.
        known_places = sorted(
            {
        ...
        single_word_known_places = [kp for kp in known_places if " " not in kp]
        ...
        def _fuzzy_known_place(token: str) -> Optional[str]:
            # Catches common single-letter OCR confusions (e.g. ب/ن, which
            # look alike and only differ by dot placement) against the
            # single-word gazetteer entries, e.g. "قانس" -> "قابس" (Gabes).
            if len(token) < 3:
                return None
            for kp in single_word_known_places:
                if _edit_distance_at_most_one(token, kp):
                    return kp
            return None
        ...
        def clean_place_candidate(raw: str) -> Optional[str]:
            ...
            for token in raw_norm.strip().split():
                ...
                if clean in likely_name_tokens and tokens:
                    break
                # A near-miss (single-letter OCR confusion) against a known
                # single-word place is treated as that place directly,
                # ignoring any further accumulated tokens: those are almost
                # certainly an unrelated fragment glued on by the raw_text
                # concatenation (e.g. "قانس اعي" -> "قابس", dropping "اعي",
                # a leftover piece of a nearby family-name value).
                fuzzy = _fuzzy_known_place(clean)
                if fuzzy and not tokens:
                    return fuzzy
                tokens.append(clean)
        ...
        for idx, match in enumerate(re.finditer(pattern, text)):
            ...
            # Try the captured phrase both as OCR'd and with its word order
            # reversed: RTL box-ordering issues affect a single field's
            # phrase (e.g. "الكبرى القلعة" instead of "القلعة الكبرى"), not
            # necessarily the whole raw_text, so reversal is attempted here
            # per-match rather than on the entire flattened text. The
            # reversed attempt is scored slightly lower so the OCR'd order
            # wins on ties, but a genuine gazetteer match (+15 bonus below)
            # from the reversed form still wins over a non-gazetteer guess.
            variants = [(captured, 0, "place_after_label")]
            reversed_phrase = " ".join(reversed(captured.split()))
            if reversed_phrase != captured:
                variants.append((reversed_phrase, 3, "place_after_label_reversed"))

            for phrase, penalty, reason_kind in variants:
                place = clean_place_candidate(phrase)
                if not place:
                    continue

                score = 40 + idx - penalty
                reason = f"{reason_kind}:{match.group(0)}"
    ...
    def _looks_like_family_name(self, value: Any) -> bool:
        ...
        FIX v2.2: reject candidates containing a duplicated token (e.g.
        "يعامج زينب يعامج"). A repeated token inside the same short
        (<=3 word) candidate is a strong signal of a concatenation artefact
        from juxtaposed multi-engine OCR passes, not a real Tunisian family
        name — Tunisian family names never repeat the same word twice.
        """
        ...
        # FIX v2.2: reject duplicated-token artefacts.
